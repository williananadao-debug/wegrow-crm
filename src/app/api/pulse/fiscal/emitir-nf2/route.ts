...
// POST { leadId } — emite a NF2 (remessa, CFOP 5116/6116) referenciando a chave da NF1 já
// autorizada dessa venda. Chamar só no momento real da entrega do trailer.
export async function POST(req: NextRequest) {
  ...
  const itensLead = (Array.isArray(lead.itens) ? lead.itens : []) as { servico: string; quantidade: number; precoUnitario: number }[];
  ...
  if (semNcm.length > 0) {
    return NextResponse.json({ error: `Produto(s) sem NCM cadastrado: ${semNcm.map(i => i.servico).join(', ')}.` }, { status: 400 });
  }
  ...
  const { data: notaRascunho, error: erroInsert } = await db.from('fiscal_notas').insert([{
    ...
    lead_id: leadId, ref_focus_nfe: ref, chave_nf_referenciada: nf1.chave_acesso, valor_total: lead.valor_total,
    cnpj_participante: soDigitos(cliente.cnpj), nome_participante: cliente.nome_empresa,
    observacao: `NF2 (remessa, entrega futura) — referencia NF1 ${nf1.chave_acesso}. Aguardando autorização da SEFAZ.`,
  }]).select('id').single();
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : 'Falha ao emitir a NF2.';
