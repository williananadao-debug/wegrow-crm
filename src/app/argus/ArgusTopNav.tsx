...
// dourado (licitação, tema próprio) ou verde WeGrow padrão (veículos).
function MarcaEmpresaArgus({ logoUrl, inicial, isVeiculos }: { logoUrl?: string | null; inicial: string; isVeiculos: boolean }) {
  if (logoUrl) {
    return (
      <div className={`rounded-xl flex items-center justify-center overflow-hidden flex-shrink-0 w-10 h-10 ${isVeiculos ? 'bg-white' : 'bg-white border border-[#e5e0d5]'}`}>
        <img src={logoUrl} alt="" className="w-full h-full object-contain p-1" />
      </div>
    );
  }
  return (
    <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${isVeiculos ? 'bg-[#22C55E]' : 'bg-[#241c14]'}`}>
      <Radar size={19} className={isVeiculos ? 'text-[#0B1120]' : 'text-[#d9861c]'} />
    </div>
  );
}
...
export default function ArgusTopNav({ nomeEmpresa }: { nomeEmpresa?: string }) {
  ...
  // abas E a paleta (dourado/creme da licitação vs navy/verde padrão WeGrow dos veículos —
  // GB Motors não pediu identidade visual própria como o Foscarini pediu).
  ...
  return (
    <nav className={isVeiculos ? 'bg-[#0B1120] border-b border-white/10 sticky top-0 z-30' : 'bg-white border-b border-[#e5e0d5] sticky top-0 z-30'}>
      <div className="max-w-[1400px] mx-auto px-6 h-[72px] flex items-center gap-3">
        <div className="flex items-center gap-3 mr-6 flex-shrink-0">
          ...
          <div className="leading-tight">
            <p className={`text-[15px] font-bold ${isVeiculos ? 'text-white' : 'text-[#241c14]'}`}>{nomeEmpresa || 'Argus'}</p>
            <p className={`text-[13px] font-semibold uppercase tracking-wide ${isVeiculos ? 'text-slate-500' : 'text-[#9a958a]'}`} style={isVeiculos ? undefined : { fontFamily: 'var(--font-argus-serif)' }}>
        ...
        <div className="flex items-center gap-1 flex-1 overflow-x-auto">
          {itens.map(item => {
            ...
            const classesAtivo = isVeiculos
              ? (ativo ? 'bg-[#22C55E]/10 text-[#22C55E]' : 'text-slate-400 hover:bg-white/5 hover:text-white')
        ...
        {outrosModulos.length > 0 && (
          <div className="relative flex-shrink-0" ref={ref}>
            <button onClick={() => setOutrosAbertos(v => !v)}
              className={`flex items-center gap-1.5 px-3 py-2.5 rounded-lg text-[14px] font-semibold transition-all ${isVeiculos ? 'text-slate-400 hover:bg-white/5 hover:text-white' : 'text-[#6b6862] hover:bg-[#f7f6f3] hover:text-[#241c14]'}`}>
              <Grid3x3 size={18} /> <ChevronDown size={13} className={`transition-transform ${outrosAbertos ? 'rotate-180' : ''}`} />
            </button>
            {outrosAbertos && (
              <div className={`absolute right-0 top-full mt-2 rounded-xl shadow-lg py-1.5 min-w-[200px] z-40 ${isVeiculos ? 'bg-[#0F172A] border border-white/10' : 'bg-white border border-[#e5e0d5]'}`}>
                <p className={`text-[12px] font-bold uppercase tracking-wide px-3 py-1.5 ${isVeiculos ? 'text-slate-500' : 'text-[#9a958a]'}`}>Outros módulos</p>
                {outrosModulos.map(m => {
                  const Icon = m.icon;
                  return (
                    <Link key={m.href} href={m.href} onClick={() => setOutrosAbertos(false)}
                      className={`flex items-center gap-2.5 px-3 py-2.5 text-[14px] font-semibold transition-all ${isVeiculos ? 'text-white hover:bg-white/5' : 'text-[#241c14] hover:bg-[#faf7f2]'}`}>
                      <Icon size={15} className={isVeiculos ? 'text-slate-500' : 'text-[#9a958a]'} /> {m.label}
                    </Link>
                  );
                })}
              </div>
