    ...
    <div>
      ...
      <header className="bg-[#241c14] text-white">
        <div className="max-w-[1400px] mx-auto px-6 py-14">
          ...
          <h1 className="text-5xl font-bold leading-[1.05] mb-4" style={{ fontFamily: 'var(--font-argus-serif)' }}>
            Controle total<br />das <em className="text-[#f0a94a]">Vendas</em>
          </h1>
      ...
      <main className="max-w-[1400px] mx-auto px-6 py-10">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-10">
          <Link href="/argus/vendas" className="bg-white border border-[#e5e0d5] hover:border-[#d9861c]/50 rounded-2xl p-5 transition-all flex items-center justify-between">
            <div>
              <p className="text-sm font-bold text-[#241c14] flex items-center gap-2"><Car size={16} className="text-[#d9861c]" /> Vendas</p>
              <p className="text-[12px] text-[#9a958a] font-semibold mt-1">{leads.length} lead(s) este mês</p>
            </div>
            <ChevronRight size={16} className="text-[#c9c3b5]" />
          </Link>
          <Link href="/argus/comissoes" className="bg-white border border-[#e5e0d5] hover:border-[#d9861c]/50 rounded-2xl p-5 transition-all flex items-center justify-between">
            <div>
              <p className="text-sm font-bold text-[#241c14] flex items-center gap-2"><Percent size={16} className="text-[#d9861c]" /> Comissões</p>
              <p className="text-[12px] text-[#9a958a] font-semibold mt-1">{ganhos.length} venda(s) fechada(s)</p>
            </div>
            <ChevronRight size={16} className="text-[#c9c3b5]" />
          </Link>
          <Link href="/argus/marketing" className="bg-white border border-[#e5e0d5] hover:border-[#d9861c]/50 rounded-2xl p-5 transition-all flex items-center justify-between">
            <div>
              <p className="text-sm font-bold text-[#241c14] flex items-center gap-2"><TrendingUp size={16} className="text-[#d9861c]" /> Marketing</p>
              <p className="text-[12px] text-[#9a958a] font-semibold mt-1">Redes sociais da loja</p>
            </div>
            <ChevronRight size={16} className="text-[#c9c3b5]" />
          </Link>
        </div>
        ...
        <div className="flex items-center justify-between mb-4">
          <p className="text-[13px] font-bold text-[#9a958a] uppercase tracking-widest">Movimentação recente</p>
          <Link href="/argus/vendas" className="text-[13px] font-bold text-[#d9861c] flex items-center gap-1">Ver tudo <ChevronRight size={12} /></Link>
        </div>
        ...
        {loading ? (
          ...
          <div className="bg-white border border-[#e5e0d5] rounded-2xl p-10 text-center">
            <p className="text-[#6b6862] font-semibold text-sm">Nenhum lead registrado este mês ainda.</p>
          </div>
        ) : (
          <div className="space-y-2">
            {recentes.map(l => (
              <div key={l.id} className="bg-white border border-[#e5e0d5] rounded-xl p-4 flex items-center justify-between gap-3">
                <div className="min-w-0">
                  <p className="text-sm font-bold text-[#241c14] truncate">{l.empresa}</p>
                  <p className="text-[12px] text-[#9a958a] font-semibold truncate">{l.veiculo_referencia || 'Sem veículo informado'} · {l.vendedor_nome || 'Sem vendedor'}</p>
                </div>
                <div className="flex items-center gap-3 flex-shrink-0">
                  <span className="text-sm font-bold text-[#241c14]">{fmtMoeda(l.valor_total)}</span>
