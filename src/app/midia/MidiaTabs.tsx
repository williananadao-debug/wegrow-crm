    ...
    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
      <div>
        <h1 className="text-4xl font-black tracking-tighter uppercase italic text-white flex items-center gap-3">
          <Megaphone size={28} className="text-[#22C55E]" /> Demais FM Comercial
        </h1>
        <p className="text-slate-500 text-sm font-bold uppercase tracking-widest mt-1">Painel de audiência, redes e propostas</p>
      </div>
      <div className="flex items-center gap-2 flex-wrap">
        <div className="bg-[#0F172A] border border-white/10 p-1 rounded-xl flex gap-1 flex-wrap">
          {ABAS.map(aba => {
            ...
            const Icon = aba.icon;
            return (
              <Link key={aba.href} href={aba.href} className={`flex items-center gap-1.5 px-3 py-2 rounded-lg text-[11px] font-black uppercase tracking-widest transition-all ${ativo ? 'bg-[#22C55E] text-[#0B1120] shadow-[0_0_15px_rgba(34,197,94,0.3)]' : 'text-slate-500 hover:text-white hover:bg-white/5'}`}>
