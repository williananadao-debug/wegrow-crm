      ...
      <div className="flex items-center gap-2 flex-wrap">
        <div className="bg-[#0F172A] border border-white/10 p-1 rounded-xl flex gap-1 flex-wrap">
          {ABAS.map(aba => {
            ...
            const Icon = aba.icon;
            return (
              <Link key={aba.href} href={aba.href} className={`flex items-center gap-1.5 px-3 py-2 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all ${ativo ? 'bg-[#22C55E] text-[#0B1120] shadow-[0_0_15px_rgba(34,197,94,0.3)]' : 'text-slate-500 hover:text-white hover:bg-white/5'}`}>
