...
import { Loader2, HardHat, ArrowLeft, MapPin, Calendar, ListChecks, Users, Receipt, ChevronRight, Boxes, BookOpen, FileText } from 'lucide-react';
  ...
  return (
    <div className="p-4 md:p-8 pb-20 text-white">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Link href={`/obras/${obra.id}/suprimentos`} className="bg-[#0F172A] border border-white/10 hover:border-orange-500/40 rounded-2xl p-6 transition-all flex flex-col gap-3">
          <div className="flex items-center justify-between">
            <div className="w-10 h-10 bg-purple-500/10 rounded-xl flex items-center justify-center"><Boxes size={18} className="text-purple-400" /></div>
            <ChevronRight size={16} className="text-slate-600" />
          </div>
          <div>
        ...
        <Link href={`/obras/${obra.id}/documentos`} className="bg-[#0F172A] border border-white/10 hover:border-orange-500/40 rounded-2xl p-6 transition-all flex flex-col gap-3">
          <div className="flex items-center justify-between">
            <div className="w-10 h-10 bg-cyan-500/10 rounded-xl flex items-center justify-center"><FileText size={18} className="text-cyan-400" /></div>
            <ChevronRight size={16} className="text-slate-600" />
          </div>
          <div>
            <p className="text-sm font-black text-white uppercase">Documentos</p>
            <p className="text-[10px] text-slate-500 font-bold uppercase mt-0.5">Projetos, licenças, ARTs</p>
          </div>
        </Link>
      </div>
    </div>
