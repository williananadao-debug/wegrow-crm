...
export default function Navbar() {
  ...
  // Estado para recolher no Desktop
  const [isCollapsed, setIsCollapsed] = useState(false);
  // Estado para abrir no Mobile
  const [isMobileOpen, setIsMobileOpen] = useState(false);

  // ESCUTA O EVENTO DA TOPBAR PARA ABRIR NO CELULAR
  useEffect(() => {
  ...
  return (
    <>
      {/* BACKGROUND ESCURO (Aparece no mobile quando o menu abre) */}
      {isMobileOpen && (
        <div 
      ...
      >
        ...
        <div className={`flex items-center gap-3 mb-10 text-white transition-all ${isCollapsed ? 'justify-center' : ''}`}>
          <div className="w-10 h-10 min-w-[40px] bg-[#22C55E] rounded-xl flex items-center justify-center font-black text-[#0F172A] text-xl shadow-[0_0_15px_rgba(34,197,94,0.3)]">
            W
          </div>
        ...
        <nav className="flex flex-col gap-2 h-full overflow-y-auto no-scrollbar">
          ...
          <div className="mt-auto pt-4 border-t border-white/5 space-y-2">
            ...
            >
              <Settings size={20} />
              {!isCollapsed && <span className="text-sm font-semibold whitespace-nowrap">Configurações</span>}
            </Link>
            ...
            >
              <LogOut size={20} />
              {!isCollapsed && <span className="text-sm font-semibold whitespace-nowrap">Sair</span>}
            </button>
          </div>
        </nav>
      </div>
