-- Corrige risco de a Estratégia IA gerar lead pra um vendedor que não atende aquele
-- cliente: antes, escolher um vendedor específico no dropdown (p_vendedor_id) ou a
-- distribuição automática (menos leads abertos) sobrescrevia o vendedor que já vendeu
-- pra aquele cliente (resgate/churn) ou o vendedor responsável no cadastro (mix).
-- Agora o dono já existente do cliente é a amarra: só cai no vendedor escolhido/
-- distribuição automática quando não há dono conhecido.
...
BEGIN
  ...
  -- Vendedor de fallback (só usado quando o cliente não tem dono conhecido):
  -- específico ou quem tem menos leads abertos
  ...
  IF p_tipo = 'resgate' THEN
    ...
    SELECT
      ...
      'aberto',
      0,
      COALESCE(sub.user_id_orig, v_destino),
